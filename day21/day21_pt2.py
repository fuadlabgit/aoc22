...
class MyInt:
    ...
    def __add__(self,other):
        return MyInt("(%s + %s)" % (self.x, other.x))
...
class Monkey:
    ...
    def assign_job(self,job):
        ...
        if isinstance(self.job, int) or self.job == "x":
            self.job = MyInt(self.job)
    ...
    def check_done(self):
        ...
        if len(self.wait_for) == 0: # self.job[0] in val_dict and self.job[1] in val_dict:
            ...
            if self.job[2] == "-":
                ...
                lhs = sp.simplify(u.x)
                rhs = sp.simplify(v.x)
                sympy_eq = sp.sympify("Eq(%s, %s)" % (lhs, rhs))
                sol = sp.solve(sympy_eq)
                print("solution",sol)
    ...
    def report(self,val):
        ...
        self.__class__.values[self.code.strip()] = val
...
for line in lines:
    ...
    if len(job.split(" + ")) > 1 or  len(job.split(" / ")) > 1 or  len(job.split(" * ")) > 1 or  len(job.split(" - ")) > 1:
        ...
        my_monkey = Monkey.instances[monkey]
        my_monkey.report(MyInt(job)) # <- just eval the integer
